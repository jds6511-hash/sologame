...
- **의존 문서**:
  ...
  - `docs\art\art-pipeline-candidates.md` (현재 zoom 4 기준과 G3 이후 zoom 3 대표 샘플 비교·전환 게이트)
  - `docs\design\M2_PLAN.md` (태스크 MP-2)
- **변경 이력**:
  - 2026-09-16: 본 문서가 Phase C 승인 전까지의 현재 Godot 임포트 기준임을 명시했다. `Camera2D.zoom=3`은 G3 이후 대표 샘플 비교 후보이며 아직 씬 기본값이 아니다.
  - 2026-07-16: 최초 작성 — MP-2(x4 정수 스케일·픽셀 스냅 프로젝트 설정) 적용, `project.godot` 수정 근거 기록
  - 2026-07-18: 참조 — `STYLE_GUIDE.md` 1-2장이 디렉터 지시(G2-2)로 스프라이트 규격(인간형 16x32 등)을 개정. 본 문서의 뷰포트 1280x720 + Camera2D.zoom=4 구조는 스타듀밸리와 이미 동일함이 확인되어(STYLE_GUIDE 1-2-1절 근거표) 변경 없음
  - 2026-07-18 디렉터 지시: 해상도 1280x720 → **1920x1080** 전환(`ux-foundation.md` 5-0절 재기준 근거). 1장 표의 뷰포트·UI 렌더링 값과 실효 가시 영역을 1920x1080/480x270으로 갱신 — Camera2D.zoom=4 구조 자체는 변경 없음(뷰포트만 x1.5 커져 실효 가시 영역도 x1.5 확대)

---

## 1. 해상도·스케일 구조 (현재 런타임 확정)

`STYLE_GUIDE.md` 1-1장에 따라 **뷰포트 자체를 320x180으로 낮추지 않는다.** 대신 다음 구조를 채택한다.

이 절의 `zoom=4`는 G3 판정 기준이며 현재 모든 월드 씬에 적용한다. G3 이후 [아트 제작·전환 파이프라인](art-pipeline-candidates.md)의 대표 샘플에서 `zoom=3`을 같은 장면으로 비교하되, 디렉터 승인 전에는 기존 씬·콜리전·전투 수치를 일괄 변경하지 않는다.

| 항목 | 값 | 비고 |
|---|---|---|
| 프로젝트 뷰포트(네이티브) | **1920x1080** (2026-07-18 1280x720에서 상향) | `ux-foundation.md`의 HUD 좌표·글자 크기가 이 해상도 기준이므로 일치 |
| 월드 카메라 줌 | **Camera2D.zoom = Vector2(4, 4)** | 씬마다 카메라에 설정하는 값 (프로젝트 전역 설정 아님). 해상도 전환과 무관하게 유지 |
| 월드 실효 가시 영역 | **480x270** | 1920/4 x 1080/4 — 카메라 줌으로 달성, 뷰포트 해상도 변경이 아님 |
| UI 렌더링 | **CanvasLayer, 네이티브 1920x1080** | Camera2D 줌의 영향을 받지 않음 (분리 계층) |
...
```
[MP-2 검증] 뷰포트(네이티브 UI 기준) 크기: (1920.0, 1080.0)
[MP-2 검증] Camera2D.zoom: (4.0, 4.0)
[MP-2 검증] 월드 실효 가시 영역: (480.0, 270.0)
```
...
| 섹션 | 키 | 값 | 목적 |
|---|---|---|---|
| `[display]` | `window/size/viewport_width` / `viewport_height` | 1920 / 1080 | 네이티브 UI 해상도 (2026-07-18 디렉터 지시로 1280x720에서 상향, `ux-foundation.md` 5-0절과 일치) |
...
- 새 월드 씬(맵·전투장 등)을 만들 때 **Camera2D를 반드시 배치하고 현재 기준인 `zoom = Vector2(4, 4)`로 설정**한다 (`current = true` 포함). Phase C 대표 샘플은 별도 비교 씬/분기에서 zoom 3을 시험한다.
