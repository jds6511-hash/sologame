...
- **변경 이력**:
  ...
  - **2026-07-30 (audio-designer): 생성 자동화 완료 — 우선순위 1의 6곡 실제 생성.** 본 문서 2장 프롬프트를 실행용 표로 옮긴 스크립트 `godot\assets\tools\lyria_bgm_gen.py`로 API 생성 + 3장 후처리를 자동화했다. **1회 생성 = 30초라는 전제가 틀렸다** — `lyria-3-pro-preview`가 약 95~156초를 반환하므로 루프 트랙은 전부 pro로 전환했다(6장 신설). 3장 후처리 절차에 **"페이드 제거를 루프 접합보다 먼저"** 단계를 추가했다(모델이 `no fade` 지시를 지키지 않음 — 6-1절). 결과·실측·잔여 한계는 `ASSET_SOURCES.md` 8-3·8-4장이 마스터이며 본 문서는 프롬프트 원문 보관과 6장 실측 요약만 담당한다.

> **범위**: 본 문서는 리리아 3 생성을 위한 **입력 시트**다. 생성은 `lyria_bgm_gen.py`가 본 문서 2장 프롬프트를 그대로 사용해 수행하며(디렉터 수동 조작 불요), audio-designer가 3장 후처리 후 4장 방침에 따라 `ASSET_SOURCES.md`에 기록한다.
> **리리아 3 제약 전제**: ~~1회 생성 = 약 30초 클립~~ → **6장 실측으로 정정**(모델에 따라 30초 또는 약 2분 30초). 가사 없는 순수 기악(instrumental)만 요청, 칩튠/8비트 톤을 프롬프트 문구로 강하게 지정(신스 파형·템포·악기 편성을 명시하지 않으면 일반 오케스트라 톤으로 나올 위험이 있음).
...
리리아 3 출력은 그대로 루프시킬 수 없으므로, 게임에서 실제로 반복 재생 가능한 BGM으로 만들려면 아래 절차를 곡마다 수행한다.

> **2026-07-30 자동화**: 아래 1~6번은 `godot\assets\tools\lyria_bgm_gen.py`가 전부 수행한다(수동 작업은 7번 QA 청취만 남는다). **2번 앞에 "0번 = 페이드 구간 제거"가 추가됐다** — 6-2절 실측대로 모델이 `no fade` 지시를 무시하므로, 페이드를 남긴 채 접합하면 이음매가 무음으로 꺼진다. 최종 순서는 **페이드 제거 → 루프 접합 → 정규화 → 변환**이다.
...
- **우선순위 1의 6곡 청취 판정** (신규): 6-3절의 루프 이음매 위화감과 곡별 지역 무드 정합은 청취만이 판정 수단이다. 이음매가 거슬리는 곡은 `python godot\assets\tools\lyria_bgm_gen.py <트랙key> --force`로 다른 테이크를 뽑는 것이 가장 저렴하다(1곡 약 1분).
...
## 6. 리리아 3 실측 (2026-07-30, 우선순위 1 생성 결과)

시트 작성 시 추정이었던 제약을 실측으로 정정한다. 상세는 `ASSET_SOURCES.md` 8-3장.

### 6-1. 모델 선택 — 루프 트랙은 pro

| 모델 | 출력 길이 | 포맷 | 호출 소요 | 용도 판정 |
|---|---|---|---|---|
| `lyria-3-clip-preview` | 약 **30.8초** | 192kbps MP3, 44.1kHz 스테레오 | 28~34초 | **짧은 논루프 트랙만** (스팅어·팡파레) |
| `lyria-3-pro-preview` | 약 **95~156초** (프롬프트에 따라 가변, 길이 지정 불가) | 동일 | 38~64초 | **루프 트랙 전부** |

- **30초 클립은 게임 루프 BGM으로 쓰지 말 것.** 필드곡은 플레이어가 한 지역에서 수십 분을 보내므로 30초면 시간당 120회 반복이다. pro의 약 2분 30초는 같은 시간에 24회로 줄어 체감이 크게 다르다. 비용 차이가 문제되지 않는 한 루프 트랙은 pro가 기본이다.
- **WAV 요청은 불가**: `response_format={"type":"audio","mime_type":"audio/wav"}`는 clip·pro 공통으로 400 `Audio mime_type is not supported in response_format.` 두 모델 모두 MP3만 반환하므로 MP3 → WAV 디코드 → 후처리 → OGG 경로가 강제된다(192kbps 원본이라 실용상 문제없음).
- **쿼터**: 7회 연속 생성(clip 2 + pro 5, 약 21MB) 동안 **429/쿼터 오류 0건**. 무료 티어 상한값은 미도달이라 확인되지 않았다.

### 6-2. 프롬프트 준수도 — 지키지 않는 항목

| 지시 | 준수 여부 |
|---|---|
| 조성·템포·악기 편성(칩튠 파형 지정) | 지킨다 — 시트 문구를 그대로 쓰면 된다 |
| **`no fade in or fade out` / `seamless loop`** | **안 지킨다** — pro 전곡이 곡 끝을 무음까지 페이드아웃했다(제거량 실측 0.47~4.06초) |
| **`about 5 to 8 seconds`**(팡파레) | **안 지킨다** — 30초를 채워 보낸다 |

따라서 3장 후처리는 아래 **순서**가 필수다. 페이드를 남긴 채 루프하면 이음매에서 소리가 꺼졌다 커지는 것이 명확히 들린다.

> **1) 페이드 구간 제거 → 2) 루프 크로스페이드 접합 → 3) 음량 정규화 → 4) OGG 변환**

보정 효과(꼬리 세기비 = 마지막 0.25초 RMS / 본문 RMS, 1.0이면 페이드 없음): 타이틀 **0.035 → 0.48**, 노베라 **0.046 → 0.452**, 시작 필드 **0.859 → 1.102**.

### 6-3. 남는 한계 — 루프 이음매는 "음악적으로" 매끄럽지 않다

크로스페이드 길이를 0.6~2.4초 후보 중 꼬리·머리 상관계수가 최대인 값으로 자동 선택했으나, **실측 상관은 0.018~0.248로 낮다**. 리리아 클립의 시작과 끝은 화성적으로 무관하게 생성되므로, 이음매에 **클릭·팝은 없지만 화성이 바뀌는 전환처럼 들릴 가능성이 높다**. 이를 근본 해결하려면 (a) 같은 프롬프트로 여러 테이크를 뽑아 이음매가 가장 자연스러운 것을 고르거나, (b) 프롬프트에 `[0:00 - 0:10]` 형식의 타임스탬프 구조 제어로 시작·끝 화성을 같게 유도하는 시도가 필요하다 — 둘 다 **청취 판정이 전제**라 audio-designer 단독으로는 진행할 수 없다.
