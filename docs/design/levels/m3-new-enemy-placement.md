...
- **변경 이력**:
  - 2026-08-02: 6-1장에 재스폰 배선 한 줄 추가(마스터는 `m3-respawn-spec.md`). **마커 배치·무리 구성·좌표는 변경 없음** — D-3은 재스폰 규칙만 추가했다.
...
- **재스폰(D-3 추가)**: 각 마커는 재스폰 슬롯 1개가 되어 무리 전멸 시 쿨다운(일반 30초 / 정예 캠프 180초) 후 무리 단위로 다시 스폰된다. 규격·근거·페이스 검산은 **`docs\design\levels\m3-respawn-spec.md`가 마스터**다. 본 장의 마커 배치·무리 구성은 D-3에서 **변경되지 않았다**.
