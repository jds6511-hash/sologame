...
## "결과값"을 담는 그릇이다. 레벨업에 따른 성장 계산 자체는 이 태스크(CB-3/CB-4/CB-7)의
## 범위 밖이므로— 아직 성장 시스템이 구현되지 않았다— 지금은 Lv1 고정값을 담은
## .tres(godot/data/combat/)로 자리를 잡아 두고, 성장 시스템이 구현되면 이 리소스를
## 매 레벨 동적으로 갈아끼우거나 채워 넣는 방식으로 대체하면 된다.
...
@export var max_hp: float = 0.0  ## growth.md 1-2장
