...
## 무시하고, 아래 "정예 그로기" 그룹의 그로기 게이지를 사용한다(정예 공용 프레임,
## m3-monster-spec 7-5장). 야간 배율은 정예에도 적용된다(보스만 제외).
@export var is_elite: bool = false

@export_group("정예 그로기 (combat.md 5-2 정예·보스 행 — 정예 공용 프레임)")
## 그로기 게이지 최대치 = 유효 최대 HP × 이 비율. 누적 단위는 실제로 들어간 최종 피해량이다
## (monster_base.gd take_damage 참고).
##
## 0.25 산출 근거 (ai-dev — 기획서에 최대치 수치가 없어 확정 수치에서 역산, 디렉터 결정 아님):
## combat.md 2-1·8-1의 정예 규격은 "HP = 잡몹 ×6, 목표 TTK 약 30초"다. 즉 플레이어 지속 화력은
## "정예 최대 HP를 30초에 깎는 크기"로 이미 고정돼 있으므로, 게이지 최대치를 HP 비율로 두면
## 만충까지 걸리는 시간이 곧 (비율 × 30초)가 되어 레벨·야간 배율(×1.2)·정예 배율과 자동으로
## 정합된다(레벨별 수치표 불요). 0.25 → 약 7.5초마다 그로기 1회, 30초 전투에서 3회
## (7.5/15/22.5초 지점) 열리고 각 3초씩이므로 무방비 구간 비중은 최대 9/30초 = 30%다.
## "탱키해서 먼저 끊기 어렵지만 주기적으로 딜 타임이 열린다"(m3-monster-spec 7-4 설계 의도)를
## 만족하는 최소 빈도이며, 플레이어 스킬 축적(난입 강타 등)이 더해지면 더 앞당겨진다.
@export var groggy_gauge_hp_ratio: float = 0.25
## 그로기 지속 (m3-monster-spec 7-4 · combat.md 5-2 확정 수치 "3초 그로기(무방비)")
@export var groggy_duration_sec: float = 3.0
