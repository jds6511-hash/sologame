## 타격 이펙트 — 중 (AR-4). HitFeedbackPreset "중" 슬롯(hitfeedback_medium.tres) 대응.
##
## 재생 시간: 0.5초 이내 (STYLE_GUIDE.md 6-2장 "일반 스킬 ≤0.5초").
## 파티클 수: 이미터당 ≤32 (STYLE_GUIDE.md 6-2장 "일반 스킬 ≤32") — 본 씬은 Burst 22 + Spark 8.
## 색: 플레이어 타격 이펙트 색 위계(STYLE_GUIDE.md 6-1 순위3) #0099db/#2ce8f5 + 흰색 하이라이트.
## 예고색(#ff0044)은 어디에도 사용하지 않는다.
##
## 트리거 방법 (둘 중 하나):
##   1) (권장) HitFeedbackManager 경유 — data\combat\hitfeedback_medium.tres의 vfx_scene에
##      본 씬(res://scenes/vfx/hit_effect_medium.tscn)이 이미 연결되어 있으므로,
##      HitFeedbackManager.play(medium_preset, at_position, target) 호출 시 자동 재생된다.
##   2) 독립 재생:
##      var vfx := preload("res://scenes/vfx/hit_effect_medium.tscn").instantiate()
...
const DURATION_SEC: float = 0.5
const PARTICLE_TEXTURE_SIZE_PX: int = 7
