## 직업 선택 화면 (M3 전직 UI) — 모험가 -> 1차 직업(전사/궁수) 선택 모달.
##
...
##
## 직업 카드는 씬에 고정하지 않고 `available_jobs`로 런타임 생성한다 — 직업이 늘어도
...
## 전직 가능 상태에서만 열린다 — 조건은 B-5의 상태를 그대로 신뢰한다.
func open() -> void:
	if _transition == null or _transition.is_transitioned:
		return
	if not _transition.transition_available:
		return
	visible = true
...
func _build_cards() -> void:
	...
	if _transition == null:
		return
	for i in _transition.available_jobs.size():
		var job_def: JobDefinition = _transition.available_jobs[i]
		if job_def == null:
			continue
