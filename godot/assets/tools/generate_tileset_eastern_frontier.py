...
- 3-2장 (2026-08-20 개정): 명암 단계 상한이 대상별로 삼원화됐다.
  · **대면적 지면 9종은 3단 램프 규정 비적용** — 3-2-3절이 대체한다(아래).
    2026-08-24 정정으로 river_water 가 지면 분류에서 빠져 10종 → 9종이 됐다.
  · 프롭·구조물(rubble_stone / stone_wall / wood_fence / flag_banner / rock_small /
    bush_small **+ river_water**)은 **3단 램프 고정을 그대로 유지하고 대비를 낮추지
    않는다**(3-2-3-6절). 이 개정의 목적은 대비를 없애는 것이 아니라 지면에서 프롭으로
    소유권을 옮기는 것이다. flag_banner 는 명시적 예외(포인트색 `#fee761`, 폭 축소 금지).
- 3-2-3장 (대면적 지면 명도 대비 규칙 — 본 스크립트의 핵심 제약):
  · 게이트 지표 = 면적가중 명도 표준편차 **σ_L ≤ 10** (예외·완화 없음).
    예산식 `Σ p·Δ² ≤ 100` 과 동치이고, 검증은 `validate_ground_contrast.py` 가 한다.
  · 처방이 **재질별로 택일**된다(2026-08-24 개정).
    (B) 교차 색조 — 흙·모래·석재·목재 7종. 타일당 총 4색 상한, 기본색 1(56% 이상) +
        등명도 변주 최대 2색 + 단계색 1(통상 6~10%).
    (D) 단색조 — **초원 2종**. 색조가 재질 이름 자체인 지면은 등명도 변주를 **쓰지
        않는다**. 기본색 90% 이상 + 동일 색조 단계색 1색(6~9%) (+ 꽃 0.8% 이하).
        (B) 를 초원에 적용한 구현본은 σ_L 1.35 로 10종 최우수였으나 초록으로 읽히지 않는
        **반점밭**이 되어 디렉터가 반려했다 — 지표 통과가 룩 통과를 뜻하지 않는다.
  · 단일 색 Δ 절대값 **하드 상한 40**, 대면적 지면 `#ffffff` 금지 — (B)·(D) 공통.
  · 질감은 명암 램프가 아니라 (B)는 등명도 색상 변주, (D)는 단계색의 **불규칙 산발 +
    밀도 변화**로 만든다. 그래서 지면 타일에는 광원 방향 표현이 없다 — 방향성 음영은
    프롭이 담당. **σ_L 을 상한보다 더 낮추는 자발적 최적화는 금지**된다(3-2-3-2절).
  · 규칙적 2x2 체커 금지(정수 배율 ×4 스크롤에서 패턴 크롤링이 생긴다) —
    `texture_field()` 의 부드러운 결정론 노이즈 + 순위 기반 배분으로 면적을 정확히
    맞추면서 덩어리진 불규칙 얼룩을 만든다.
...
GREEN = RGB["green"]  # #3e8948 L107.2 초원 기본 (단색조 처방의 색조 정체성 전담)
DEEP_GREEN = RGB["deep_green"]  # #265c42 L 72.9 초원 단계색 / 프롭(덤불) 그림자
BRIGHT_YELLOW = RGB["bright_yellow"]  # #fee761 L222.6 개척 깃발 포인트색
PINK = RGB["pink"]  # #f6757a L156.1 들꽃(미세 프롭, 면적 0.8% 캡)
SAND_CREAM = RGB["sand_cream"]  # #ead4aa L213.8 모래 기본
LIGHT_APRICOT = RGB["light_apricot"]  # #e8b796 L193.9 모래·석재 등명도 변주
BLUE = RGB["blue"]  # #0099db L114.8 물 기본(프롭 취급 — 3-2-3-6 예외 1)
NAVY = RGB["navy"]  # #124e89 L 66.8 물 그림자
BLUE_GRAY = RGB["blue_gray"]  # #8b9bb4 L153.1 석재 기본(프롭) / 수면 반짝임
DARK_BLUE_GRAY = RGB["dark_blue_gray"]  # #5a6988 L104.0 프롭 그림자
...
# ---------------------------------------------------------------------------
# 대면적 지면 9종 (STYLE_GUIDE 3-2-3-5 확정 배합표 — 면적은 256px 기준 픽셀 수)
# ---------------------------------------------------------------------------


def grass_step_positions(count: int) -> list[Pos]:
    """초원 단계색 `#265c42` 의 자리를 `count`개 고른다.

    (D) 단색조 처방에서 단계색은 **질감과 형태 신호를 겸임**하는 유일한 수단이므로
    3-2-3-4 (C) 표가 "**불규칙 산발 + 밀도 변화**(풀이 짙게 뭉친 자리), 균일 산포 금지"
    를 요구한다. 그래서 굵은 스칼라장(= 뭉치는 자리를 정하는 밀도장)과 백색 노이즈
    (= 낱픽셀 산발)를 섞어 순위를 매긴다 — 밀도장 단독이면 통짜 얼룩(구멍처럼 보인다),
    노이즈 단독이면 균일 산포(반점 노이즈)가 되어 둘 다 반려 사유다.

    밀도장 셀 5px·가중 0.6 은 실측으로 고른 값이다 — 셀을 8px 로 키우면 저주파가 되어
    단계색이 **타일 가장자리에 몰리고**, 타일을 이어 붙일 때 16px 주기 띠가 생긴다
    (3-2 표가 금지한 패턴 크롤링과 같은 결함)."""
    density = texture_field(seed=131, cell_x=5, cell_y=5)
    score = {p: 0.6 * density[p] + 0.4 * noise(p[0], p[1], 133) for p in all_positions()}
    return sorted(all_positions(), key=lambda p: (-score[p], p[1], p[0]))[:count]


def grass_tile(step_count: int) -> Image.Image:
    """초록 기본색으로 타일을 채우고 단계색을 `step_count`px 얹는다.
    **등명도 변주색은 넣지 않는다** — (D) 처방의 요지이고 4번째 색 추가는 반려됐다."""
    img = new_tile()
    for pos in all_positions():
        px(img, *pos, GREEN)
    for pos in grass_step_positions(step_count):
        px(img, *pos, DEEP_GREEN)
    return img


def tile_grass_base() -> Image.Image:
    """초원. 2026-08-24 정정으로 **(D) 단색조 처방**을 쓴다 — 초록 계열은 EDG32 내
    최소 인접 단차가 34.3이라 램프가 불가능한데, 2026-08-20 개정이 이를 등명도 교차
    색조(`#5a6988` 30% + `#be4a2f` 10%)로 우회해 σ_L 1.35(10종 최우수)를 얻고도
    **초원이 초록으로 읽히지 않는 반점밭**이 되어 디렉터가 반려했다.

    초록 계열만으로 상한 10을 통과하는 배합은 전수 계산 결과 이것 하나뿐이다 —
    단계색 상한은 **23px 하드**(24px→σ_L 10.00 경계, 25px→10.18 불통)이고 지정값
    20px 의 여유 1.5%는 **다른 색으로 쓰지 않는다**(재계산 금지 — 3-2-3-5 비고).
    배합: `#3e8948` 92.2% (236px) / `#265c42` 7.8% (20px) -> σ_L 9.20"""
    return grass_tile(20)


# 들꽃 좌표 — **2px 고정**(면적 캡 0.8% = 2/256 = 0.78%). 캡은 Δ+51 픽셀의 ×4 스크롤
# 반짝임을 막는 유일한 조건이라 재량 대상이 아니다(1.5% → 0.8% 강화, 3-2-3-5 비고).
# 뭉치면 꽃 한 송이가 아니라 얼룩으로 읽히므로 타일 양단에 떨어뜨려 배치한다.
_FLOWERS: list[Pos] = [(3, 7), (12, 12)]


def tile_grass_flower() -> Image.Image:
    """초원 + 들꽃. grass_base 와의 유일한 구별 신호가 꽃이다.
    (D) 배합 위에서는 단계색 20px + 꽃 3px 이면 σ_L 10.75로 초과하므로
    **단계색을 18px 로 내리고 꽃을 2px 로 고정**한다. 두 값을 동시에 올리는 조합은
    없다(허용 대안은 꽃 3px + 단계색 16px 단 1건 — 3-2-3-5 비고).
    배합: `#3e8948` 92.2% (236px) / `#265c42` 7.0% (18px) / `#f6757a` 0.8% (2px)
    -> σ_L 9.86"""
    img = grass_tile(18)
    for pos in _FLOWERS:
        assert img.getpixel(pos)[:3] == GREEN, f"꽃 좌표 {pos} 가 단계색 위에 놓였다"
        px(img, *pos, PINK)
    return img
...
# ---------------------------------------------------------------------------
# 프롭·구조물 7종 (3-2-3-6: 게이트 비적용 — 3단 램프·현행 대비 유지)
# river_water 는 2026-08-24 정정으로 지면 분류에서 빠져 이쪽으로 옮겨졌다.
# ---------------------------------------------------------------------------

# 수면 반짝임 — 짧은 수평 파선(2~3px)이라 물결 방향과 어긋나지 않는다. **15px 고정**:
# 하이라이트를 6%보다 키우면 반짝임이 아니라 물이 하얗게 뜬다(3-2-3-6 예외 1).
# 낱픽셀 단독 점은 2x2 창에서 체커로 계수되기 쉬워 전부 파선으로 통일했다.
...
def tile_river_water() -> Image.Image:
    """여울목 개울물. 2026-08-24 정정으로 **대면적 지면 분류에서 제외**되고 프롭 취급
    (3단 램프)으로 바뀌었다(3-2-3-6 예외 1) — 개울은 연속 폭 1~2타일의 좁은 띠라
    "4타일 이상 연속으로 화면을 채운다"에 들지 않고, EDG32 청색 계열로는 σ_L 상한
    자체가 달성 불가능하다(최소 14.40). **σ_L 게이트·Δ 40 하드 상한·4색 상한 비적용.**

    기본색은 `#0099db` 로 **복귀**한다 — 물은 명도가 아니라 색조로만 물이 되고,
    남색 기본색(`#124e89` 62%)은 "짙은 물"이 아니라 "밤"이나 "구덩이"로 읽혔다.
    하이라이트는 `#8b9bb4`(저채도 회청)이며 **`#2ce8f5` 복귀는 금지**다 — 대비 사유가
    아니라 6장 색 위계(플레이어 스킬·마나 소유색) 사유이므로 예외로 풀리지 않는다.
    계속 적용되는 것: `#ffffff` 금지, 규칙적 체커 금지.
    배합: `#0099db` 74.2% (190px) / `#124e89` 19.9% (51px) / `#8b9bb4` 5.9% (15px)
    기록값(게이트 아님): σ_L 22.3 / span 86.3 — 프롭 폭 대역(63.9~145.3) 안이다."""
    img = new_tile()
    for x, y in _WATER_GLINTS:
        px(img, x, y, BLUE_GRAY)
    free = [p for p in all_positions() if p not in set(_WATER_GLINTS)]
    # 결을 가로로 늘인 스칼라장의 낮은 쪽에 그림자를 몰아 **흐름 방향 음영**을 만든다.
    scatter_fill(img, free, texture_field(212, cell_x=8, cell_y=3), [(NAVY, 51), (BLUE, 190)])
    return img
